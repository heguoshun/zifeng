...
type BatchWorkOrderAcceptDialogProps = {
    open: boolean;
    count: number;
    onClose: () => void;
...
export default function BatchWorkOrderAcceptDialog({
    open,
    count,
    onClose,
    onConfirm,
}: BatchWorkOrderAcceptDialogProps) {
    ...
    return createPortal(
        ...
        >
            <aside
                ...
                role="dialog"
                aria-modal="true"
                aria-labelledby="wom-batch-accept-drawer-title"
                onMouseDown={(event) => event.stopPropagation()}
            >
                <div className="pcp-drawer__head">
                    <h3 id="wom-batch-accept-drawer-title">批量验收</h3>
                    <button type="button" className="pcp-drawer__close" onClick={onClose} aria-label="关闭">×</button>
                </div>

                <div className="pcp-drawer__body pcp-drawer__body--form">
                    <p className="wom-batch-accept-summary">
                        已选择 <strong>{count}</strong> 条待验收工单，将统一提交验收结果。
                    </p>
