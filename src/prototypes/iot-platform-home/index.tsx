...
function IotPlatformHomeRoutes({ sessionUser }: { sessionUser: PlatformSessionUser }) {
    ...
    const [deviceAlarms, setDeviceAlarms] = useState(() => createInitialDeviceAlarms(createInitialProducts(), devices));
    ...
    const [largeMeters, setLargeMeters] = useState<LargeMeterDevice[]>(() => {
        const initialProducts = createInitialProducts();
        const initialDevices = seedInitialLargeMeterAreaBindings(
            createInitialDevices(initialProducts),
            initialProducts,
            createInitialAreas(),
        );
    ...
    useEffect(() => {
        setDeviceAlarms((previous) => {
            const fresh = createInitialDeviceAlarms(products, devices);
            const freshMap = new Map(fresh.map((alarm) => [alarm.id, alarm]));
            const merged = previous.map((alarm) => {
                const updated = freshMap.get(alarm.id);
                if (!updated) return alarm;
                return {
                    ...alarm,
            ...
            const existingIds = new Set(merged.map((alarm) => alarm.id));
