...
const DRILLDOWN_META: Record<SummaryDrilldownKey, { title: string; subtitle: string; exportName: string }> = {
    ...
    dataAnomaly: {
        title: '数据异常设备信息',
        subtitle: '展示缺报、解析失败或传感器故障的代表性设备。',
        exportName: '数据异常设备信息',
    },
    alarmPending: {
        title: '待处理告警设备信息',
    ...
    return (
        <div className="edo-page">
                ...
                <div className="edo-filter-help-wrap" ref={metricGuideRef}>
                    ...
                    {metricGuideOpen ? (
                        <div className="edo-metric-guide" role="dialog" aria-label="指标口径说明">
                            ...
                            <p>今日对比昨日，近7天对比此前7天，近30天对比此前30天。资产存量为当前快照，趋势类指标跟随时间筛选。</p>
                        </div>
                    ) : null}
                </div>
            </section>
                    ...
                    {overviewData.summaryMetrics.map((metric) => (
                        <article className="edo-health-summary" key={metric.label}>
                            ...
                            <div className="edo-health-summary__detail">
                                {metric.icon === 'online' ? (
                                    <>
                                        <button type="button" onClick={() => setSummaryDrilldownOpen('onlineActive')}>在线 {overviewData.onlineRows[0]?.value.toLocaleString() ?? '0'} 台</button>
                                        <span>，</span>
                                        <button type="button" onClick={() => setSummaryDrilldownOpen('onlineOffline')}>离线 {overviewData.onlineRows[1]?.value.toLocaleString() ?? '0'} 台</button>
                                    </>
                                ) : null}
                                {metric.icon === 'data' ? (
            ...
            <div className="edo-two-column">
                <section className="edo-panel" aria-labelledby="edo-assets-title">
                    ...
                    <div className="edo-proportion-layout edo-proportion-layout--asset">
                        <div
                            ...
                            aria-label={`查看各${effectiveDimension}类型设备占比`}
                        >
                            <InteractiveDonut
                                rows={assetRows}
                ...
                <section className="edo-panel" aria-labelledby="edo-online-title">
                    <div className="edo-section-head"><div><h2 id="edo-online-title">在线／离线分析</h2><p>连接健康度与长期离线风险</p></div><button className="edo-text-button" type="button" onClick={onNavigateDevices}>查看设备 <ChevronRight size={14} /></button></div>
                    <div className="edo-proportion-layout edo-proportion-layout--online">
                        <div
                            className="edo-donut-block"
                            role="button"
                            tabIndex={0}
                            ...
                            aria-label="查看在线与离线设备占比"
                        >
                            <InteractiveDonut
                                rows={overviewData.onlineRows}
                                centerValue={`${overviewData.onlineRate.toFixed(1)}%`}
                                centerLabel="设备在线率"
                                colors={['#42a5f5', '#dce3ec']}
                                ariaLabel="在线与离线设备占比"
                            />
                            <span className="edo-donut-legend">
                                <span><i className="is-primary" />在线 <b>{overviewData.onlineRows[0]?.value.toLocaleString() ?? '0'}</b></span>
                                <span><i />离线 <b>{overviewData.onlineRows[1]?.value.toLocaleString() ?? '0'}</b></span>
                            </span>
                        </div>
                        <div className="edo-online-share">
                            <div className="edo-online-change">
                                <span>在线率较上期</span>
                                <strong className={overviewData.onlineRateChangeDirection === 'down' ? 'is-negative' : undefined}>
                                    {overviewData.onlineRateChangeDirection === 'down' ? <ArrowDownRight size={14} /> : <ArrowUpRight size={14} />}
                                    {overviewData.onlineRateChange.toFixed(1)}%
                                </strong>
                            </div>
                            <div className="edo-duration">
                                <span>离线时长占比</span>
                                {overviewData.offlineDuration.map((row, index) => (
                                    <button type="button" key={row.label} onClick={onNavigateDevices}>
                                        <i className={`is-color-${index + 1}`} />
                                        <span>{row.label}</span>
                                        <strong>{row.value.toLocaleString()}</strong>
                                        <small>{row.ratio}%</small>
            ...
            <div className="edo-two-column edo-two-column--lower">
                <section className="edo-panel" aria-labelledby="edo-data-title">
                    <div className="edo-section-head">
                        <div><h2 id="edo-data-title">数据接收异常</h2><p>异常设备去重统计，异常次数累计统计 · {overviewData.anomaly.trendLabel}</p></div>
