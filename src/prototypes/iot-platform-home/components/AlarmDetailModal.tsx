...
function ConditionDetailTable({
    ...
    products: ProductRecord[];
    devices?: DeviceRecord[];
    onViewDeviceData?: (device: DeviceRecord) => void;
}) {
    const matchedDevice = resolveDeviceByAlarmCode(condition.deviceCode, devices);

    const rows = [
        {
            label: '触发设备：',
            value: condition.deviceCode,
            renderValue: matchedDevice ? (
                <button
                    type="button"
                    className="dai-link-btn"
                    onClick={() => onViewDeviceData?.(matchedDevice)}
                >
                    {condition.deviceCode}
                </button>
            ) : condition.deviceCode,
        },
        { label: '所属产品：', value: resolveConditionProductName(condition, products) },
        { label: '所属片区：', value: condition.space },
        { label: '触发条件：', value: condition.condition },
    ] as const;
    ...
    return (
        <div className="dai-condition-table">
            ...
            {rows.map((row) => (
                <div key={row.label} className="dai-condition-table__row">
                    <span>{row.label}</span>
                    <span>{'renderValue' in row ? row.renderValue : row.value}</span>
                </div>
            ))}
        </div>
    );
...
export default function AlarmDetailModal({
    ...
    const handleViewDeviceData = (device: DeviceRecord) => {
        onClose();
        navigateDeviceForm('view', { deviceId: device.id, tab: '属性数据' });
    };
                                ...
                                <div className="dai-trigger-info">
                                    ...
                                    {triggerConditions.map((condition, index) => (
                                        <ConditionDetailTable
                                            ...
                                            products={products}
                                            devices={devices}
                                            onViewDeviceData={handleViewDeviceData}
                                        />
                                    ))}
                                </div>
                            </DetailRow>
