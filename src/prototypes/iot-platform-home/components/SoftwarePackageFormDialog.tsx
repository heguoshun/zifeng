...
type SoftwarePackageFormDialogProps = {
    open: boolean;
    mode: 'add' | 'edit';
    initialValue?: SoftwarePackageFormValue;
    products: ProductRecord[];
    onClose: () => void;
    onSubmit: (value: SoftwarePackageFormValue) => void;
};

export default function SoftwarePackageFormDialog({
    open,
    mode,
    initialValue,
    products,
    onClose,
    onSubmit,
}: SoftwarePackageFormDialogProps) {
    const [form, setForm] = useState<SoftwarePackageFormValue>(EMPTY_SOFTWARE_FORM);
    ...
    useEffect(() => {
        if (!open) return;
        setForm(mode === 'edit' && initialValue ? initialValue : EMPTY_SOFTWARE_FORM);
        setTouched(false);
    ...
    const fileError = validateSoftwareFileName(form.fileName);
    ...
    return createPortal(
        <div className="pcp-drawer-mask ru-drawer-mask" role="presentation" onMouseDown={handleMaskMouseDown}>
            <aside
                ...
                role="dialog"
                aria-modal="true"
                aria-labelledby="ru-software-drawer-title"
                onMouseDown={(event) => event.stopPropagation()}
            >
                <div className="pcp-drawer__head">
                    <h3 id="ru-software-drawer-title">{mode === 'add' ? '添加软件包' : '编辑软件包'}</h3>
                    <button type="button" className="pcp-drawer__close" onClick={onClose} aria-label="关闭">×</button>
                </div>
                <div className="pcp-drawer__body pcp-drawer__body--form">
                    <label className="pcp-drawer-field">
                        <span className="pcp-form-label"><em>*</em>软件包名称</span>
                        <input
                            type="text"
                    ...
                    <label className="pcp-drawer-field">
                        <span className="pcp-form-label"><em>*</em>软件包版本号</span>
                        <input
                            type="text"
                            className="pcp-form-input"
                            placeholder="请输入版本号"
                    ...
                    <label className="pcp-drawer-field">
                        <span className="pcp-form-label">软件包描述</span>
                    ...
                    <div className="pcp-drawer-field">
                        <span className="pcp-form-label"><em>*</em>上传软件包</span>
                        <input
                            ref={fileInputRef}
                            type="file"
