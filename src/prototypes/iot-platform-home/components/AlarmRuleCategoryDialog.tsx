...
type AlarmRuleCategoryDialogProps = {
    open: boolean;
    mode: 'add' | 'edit';
    parentOptions: { label: string; value: string }[];
    initialValue?: AlarmRuleCategoryFormValue;
    onClose: () => void;
    onConfirm: (value: AlarmRuleCategoryFormValue) => void;
};

const EMPTY_FORM: AlarmRuleCategoryFormValue = {
    name: '',
    parentId: '',
};

export default function AlarmRuleCategoryDialog({
    open,
    mode,
    parentOptions,
    initialValue,
    onClose,
    onConfirm,
}: AlarmRuleCategoryDialogProps) {
    const [form, setForm] = useState<AlarmRuleCategoryFormValue>(EMPTY_FORM);
    ...
    return createPortal(
        ...
        >
            <aside
                ...
                role="dialog"
                aria-modal="true"
                aria-labelledby="arc-category-dialog-title"
                onMouseDown={(event) => event.stopPropagation()}
            >
                <div className="pcp-drawer__head">
                    <h3 id="arc-category-dialog-title">
                        {mode === 'add' ? '新增规则分类' : '编辑规则分类'}
                    </h3>
