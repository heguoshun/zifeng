...
function buildLargeMeterAlarms(productId: string): DeviceAlarmRecord[] {
...
export function createInitialDeviceAlarms(products: ProductRecord[]): DeviceAlarmRecord[] {
    ...
    const largeMeterProduct = products.find((product) => product.categoryId === 'dabiao') ?? products[0];
    const largeMeterAlarms = buildLargeMeterAlarms(largeMeterProduct.id);
